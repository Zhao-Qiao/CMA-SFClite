...
-------------------------
Applies soft gates (alpha ∈ [1.0, ..., 0.0]) on top CMA mediators using TransformerLens.
"""
            ...
            for feat_idx, feat_score in feature_list:
                for alpha in alphas:
                    ...
                    if corpus_tokens is not None:
                        hook_fn = make_scaling_hook(dictionary, [int(feat_idx)], alpha)
                    ...
                    results.append({
                        ...
                        "feature_set": json.dumps([int(feat_idx)]),
                        "feature_score": float(feat_score),
                        "feature_source": source,
                        "alpha": float(alpha),
                        "feature_count": 1,
                        "sum_abs_edit": abs(1.0 - float(alpha)),
...
def plot_local_gate_results(rows: List[Dict], csv_path: str) -> None:
    ...
    alphas_vals = [r.get("alpha", 1.0) for r in agg_rows]

    plt.figure(figsize=(8, 5))
    scatter = plt.scatter(remaining, delta_vals, c=alphas_vals, cmap="viridis", alpha=0.7)
    plt.xlabel("剩余偏差（% 基线）")
    plt.ylabel("ΔPPL（语料或均值）")
    plt.title("Local Gate (Feature Scaling)")
...
def build_pareto_frontiers(rows: List[Dict]) -> Dict[str, List[Dict]]:
    ...
    for key, group in groups.items():
        if not group:
            continue
        group_sorted = sorted(group, key=lambda g: (g.get("alpha", 0.0), g.get("feature_source", "")))
        pareto: List[Dict] = []
